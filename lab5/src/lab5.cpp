...
void ziran(vector<double> x, vector<double> (*f)(vector<double>))
{
	...
	for (int i = 0; i < x.size()-1; i++)
	{
		cout <<"x"<<i<< "= np.linspace("<<x[i]<<","<<x[i+1]<<", 256)"<<endl;
		cout <<"y"<<i<<"=(("<<x[i+1]<<"-x"<<i<<")**3*("<<M[i]<<")+("<<"x"<<i<<"-("<<x[i]<<"))**3*("<<M[i+1]<<"))/(("<<h[i]<<")*"<<6<<")+(("<<x[i+1]<<")-"<<"x"<<i<<")*("<<C[i]<<")+("<<"x"<<i<<"-("<<x[i]<<"))*("<<D[i]<<")"<<endl;
		cout <<"plt.plot(x"<<i<<",y"<<i<<",linestyle='-',color='r')"<<endl;
	}
	cout <<"x = np.array([";
	for (int i = 0; i < x.size()-1; i++)
	{
	...
	cout <<fx[x.size()-1]<<"])"<<endl;
	cout <<"plt.scatter(x, y , marker='o', s=40, color='#008000')"<<endl;
}
